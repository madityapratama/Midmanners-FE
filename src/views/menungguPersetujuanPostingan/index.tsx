...
const fetchPendingPosts = async () => {
      try {
        const response = await api.get(`${process.env.NEXT_PUBLIC_API_URL}/posts/pending`);
        const formattedPosts = response.data.data.map((post: any) => ({
          id: post.id,
          seller: post.seller.name,
          title: post.title,
          category: post.categories,
          price: `Rp${parseFloat(post.price).toLocaleString('id-ID')}`,
          status: post.status,
          date: new Date().toISOString().split('T')[0] // Using current date as example
        }));
  ...
  const handleApprove = async (id: number) => {
    try {
      await api.patch(`/posts/${id}/accepted`);
  ...
  const handleReject = async (id: number) => {
    try {
      await api.patch(`/posts/${id}/rejected`);
      setPosts(posts.map(post => 
  ...
  const goToSellerPending = (id: string) => {
    router.push(`/pendingPostingan/${id}`);
  };
